...
| `PaperEnrichmentStore` SQL (`cile-enrichment-1`) | 46 configured tables; dated runtime receipts distinguish the initial 22, normalized 38 and annotation 46. Targets, immutable inputs, sources, jobs/attempts, proposals, scoped facts and joins, receipts, documents/bibliography | Signed domain operations, private console, Worker schedule; validated atomic SQLite transactions | Private enrichment APIs and closed public projections use normalized extractions after migration 0007 | Active private enrichment authority; target bibliography is still copied from Pages |
| Candidate archive migration 0009 (nine relations) | Candidate identity, sourced bibliography/coverage revisions, repeated values, heads and receipts | Signed staged importer; real writer tested on every row in isolation | Private revision-bound export and integrity audit | Configured staging; current CSV authorities and deployed runtime remain explicitly separate until cutover gates pass |
...
| GitHub candidate issues and annotation comments | Original external inputs; GitHub IDs are source locators, not candidate identity | Existing automation and curator; governed issue-comment ingress captures immutable source revisions | Annotation acquisition and audit | Input channel; release cutover replaces browser parsing with the closed archive API, conditional on verified live migration |
| `enrichment_ingress_snapshots`, `enrichment_manual_annotations` and six related SQL tables; `ingress/*` private objects | Source captures, scoped annotation fields, explicit class assertions, head/event history and readback receipts | Signed idempotent ingest; CAS head transitions; complete-census withdrawal | Public annotation/index projectors and private audit | Governed unreviewed annotation authority; unresolved associations remain private exceptions |
...
- `review-v2-model.md` historically cites profile 0.2.0. The machine normative
  current profile is 0.4.4; historical text does not select the active storage engine.
