#!/usr/bin/env python3
"""Synchronise mechanical abstract coverage into materialised curator issues."""
...
COVERAGE_PATH = ROOT / "data" / "curation" / "abstract_coverage.csv"
SECTION_HEADING = "## Abstract coverage — mechanical"
...
def api_request(repository: str, token: str, method: str, path: str, payload=None):
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "criminal-infiltration-abstract-coverage-sync",
        },
    )
    try:
...
- Coverage status: {inline(row.get('coverage_status', ''))}
- Abstract source: {inline(row.get('abstract_source', ''))}
- Article URL: {safe_link(row.get('article_url', ''))}
- Providers tried: {inline(row.get('providers_tried', ''))}
- Match type: {inline(row.get('match_type', ''))}
- Match score: {inline(row.get('match_score', ''))}
- Provider errors: {inline(row.get('provider_errors', ''))}
- Last checked: {inline(row.get('checked_at', ''))}
- Abstract text persisted: `no`

This is mechanical coverage metadata only. It records whether the zero-cost
resolver found an abstract and where; abstract text itself is deliberately not
stored here and no editorial decision is inferred."""


def replace_section(body: str, replacement: str) -> str:
    existing = str(body or "").rstrip()
    pattern = re.compile(r"(?ms)^## Abstract coverage — mechanical\s*$.*?(?=^##\s|\Z)")
    ...
    retrieval = re.search(r"(?m)^## Retrieval coverage — mechanical\s*$", existing)
    if retrieval:
        before = existing[: retrieval.start()].rstrip()
        after = existing[retrieval.start() :].lstrip()
        return f"{before}\n\n{replacement}\n\n{after}\n"
...
def sync(repository: str, token: str, coverage_path: Path) -> dict[str, int]:
    ...
    if not rows:
        print("Abstract coverage is not materialised yet; nothing to sync.")
    ...
    for row in rows:
        ...
        updated += 1
    if missing:
        raise SyncError(f"{missing} abstract coverage rows lack a materialised curator issue")
