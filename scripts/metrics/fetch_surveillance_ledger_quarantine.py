...
The quarantine currently covers:
- two historical duplicate batches;
- the accidentally edited original terminal for intake #320;
- immutable terminals later proven schema-invalid because telemetry text exceeded
  the v3 bounds or, in one case, because the terminal timestamp preceded its
  declared window end.

Every schema-invalid terminal is replaced by a separately appended v3 terminal
with the same batch identity, status, metrics, provider provenance and intake
reference; only bounded telemetry text is normalised. Scientific/intake content
is never withdrawn or changed by this wrapper.
