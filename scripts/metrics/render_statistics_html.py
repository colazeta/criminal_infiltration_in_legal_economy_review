...
def run_counts(row):
    """Use only recorded counts; missing values are not measured zeroes."""
    values = []
    if row.get('queriesCompleted') is not None and row.get('queriesPlanned') is not None:
        values.append(f"Query: {row['queriesCompleted']}/{row['queriesPlanned']}.")
    for key, label in [('uniqueResults', 'Risultati unici'), ('intakeCandidates', 'Nuovi candidati segnalati')]:
...
def render_statistics_page(path, payload):
    ...
    if rows:
        ...
        parts.append('Ultima giornata ordinaria pubblicata: ' + datetime.fromisoformat(last['date']).strftime('%d/%m/%Y') + '. ' + run_counts(last))
    if extras:
        ...
        parts.append(f'Ultima ricerca straordinaria pubblicata: {when} (ora di Roma). ' + run_counts(last))
