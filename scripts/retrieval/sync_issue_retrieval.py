...
def issue_inventory(repository: str, token: str) -> dict[str, dict]:
    ...
    # Created-ascending order prevents newly-created issues from shifting already
    # consumed pages. The identity check below also tolerates an identical API row
    # repeated across a page boundary while retaining fail-closed duplicate checks.
    for issue in paginated(
        ...
        # Candidate IDs include lowercase hexadecimal batch fragments.
