...
def api(resource, *, method="GET", payload=None):
    ...
    if not account or not token:
        raise RuntimeError("Cloudflare account or deploy credential unavailable")
    request = Request(f"https://api.cloudflare.com/client/v4/accounts/{account}/{resource}",
    ...
    try:
        ...
        raise RuntimeError(f"Cloudflare {resource.split('/')[0]} permission/request failure: HTTP {error.code}") from None
    if not result.get("success"):
        raise RuntimeError(f"Cloudflare {resource.split('/')[0]} operation failed")
