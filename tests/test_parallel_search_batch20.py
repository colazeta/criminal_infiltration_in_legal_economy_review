...
class ParallelSearchBatch20Tests(unittest.TestCase):
    ...
    def test_full_text_bridge_is_materialized_only_for_champeyrache(self) -> None:
    ...
    def test_ojp_abstract_source_is_materialized_without_full_text_promotion(self) -> None:
        row = self.abstract[OJP_INDICATORS]
        self.assertEqual(row["coverage_status"], "available")
        self.assertEqual(row["match_type"], "verified_abstract_source")
        self.assertEqual(row["article_url"], OJP_INDICATORS_URL)
