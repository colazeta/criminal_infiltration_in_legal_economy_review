...
class ArchitectureCatalogueTests(unittest.TestCase):
    def test_every_sql_column_has_a_semantic_trace_without_runtime_inference(self):
        ...
        self.assertEqual(len(physical['tables']), 88)
        self.assertEqual(len(trace), 721)
        self.assertFalse(physical['runtime_verified'])
        self.assertEqual(len([t for t in physical['tables'] if t['implementation_status'] == 'configured_enrichment_sqlite']), 55)
