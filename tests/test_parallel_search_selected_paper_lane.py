...
class ParallelSearchSelectedPaperLaneTests(unittest.TestCase):
    ...
    def test_tenth_parallel_search_batch_promotes_three_exact_full_text_sources(self):
        payload = json.loads((ROOT / "data/curation/reading_aid_overrides.json").read_text(encoding="utf-8"))
        records = {row["candidateId"]: row for row in payload["records"]}
        with (ROOT / "data/curation/review_queue.csv").open(encoding="utf-8", newline="") as handle:
            queue_ids = {row["candidate_id"] for row in csv.DictReader(handle)}
        ...
        expected = {
            "CAND-ACADEMIC-2026-09-10-EXTRA-dbc5cb0d9888-026": "lagv2019.sciencesconf.org",
            "CAND-ACADEMIC-2026-09-10-EXTRA-dbc5cb0d9888-031": "www.publicsafety.gc.ca",
            "CAND-ACADEMIC-2026-09-10-EXTRA-f276bc827f57-001": "www.ojp.gov",
        }
        for candidate, host in expected.items():
            self.assertIn(candidate, queue_ids)
            row = records[candidate]
            self.assertEqual(row["kind"], "full_text_intro")
            self.assertTrue(row["sourceUrl"].startswith("https://"))
            ...
            self.assertIn("Parallel Search", row["note"])
            self.assertIn("Evidence basis: full_text", row["note"])
            self.assertIn("private full-text ingestion", row["note"])

        brief = records["CAND-ACADEMIC-2026-09-10-EXTRA-dbc5cb0d9888-031"]
        self.assertIn("Research Brief no. 26", brief["sourceLabel"])
        self.assertIn("related full report", brief["note"])
        self.assertIn("not silently substituted", brief["note"])
        moore = records["CAND-ACADEMIC-2026-09-10-EXTRA-f276bc827f57-001"]
        self.assertIn("106775NCJRS.pdf", moore["sourceUrl"])
        self.assertIn("Harvard", moore["note"])
