...
Profile 0.4.3 governs observed GitHub source snapshots and unreviewed reading
...
authority and scientific review state are separate. Missing/conflicting markers
remain retained exceptions; no study, work identity or scientific approval is
created by parsing an annotation. See `docs/architecture/annotation-archive.md`.
...
controlled vocabularies and profile 0.4.1 do not change. Historical two-source
...
one preserved receipt per active daily candidate. Scientific profile 0.4.1 and
...
## Scientific enrichment — profile 0.4.1
...
## Hour-40 delivery — profile 0.4.1

`modules/enrichment-schedule.json` and additive migration 0004 distinguish the
...
## Document repository — profile 0.4.4

`modules/document-repository.json` and additive migration 0009 separate retained
SourceDocument receipts from Manifestation, retain exact ExtractedDocument text,
and index DocumentPage/DocumentChunk locators. Lexical terms and bibliography
are disposable retrieval indexes, never scientific evidence or canonical works.
Public text follows the current PDF redistribution gate. See
[the implementation audit](../docs/architecture/document-repository-implementation-audit.md).
