...
export const BACKUP_CONTRACT='CILE-PRIVATE-ARCHIVE-BACKUP-1';
...
async function backupKey(secret){
  ...
  const material=await crypto.subtle.sign('HMAC',root,enc.encode(BACKUP_CONTRACT));
  return crypto.subtle.importKey('raw',material,{name:'AES-GCM'},false,['encrypt','decrypt']);
}
export async function sealBackupPage(value,secret,commit,snapshot_id){
  const raw=enc.encode(canonicalJson(value));if(raw.length>7000000)fail();
  const iv=crypto.getRandomValues(new Uint8Array(12)),aad=enc.encode(BACKUP_CONTRACT+'\n'+commit+'\n'+snapshot_id);
  const ciphertext=await crypto.subtle.encrypt({name:'AES-GCM',iv,additionalData:aad,tagLength:128},await backupKey(secret),raw);
  return {contract:BACKUP_CONTRACT,commit,snapshot_id,iv:b64(iv),ciphertext:b64(new Uint8Array(ciphertext))};
}
export async function openBackupPage(page,secret,commit,snapshot_id){
  if(!page||Object.keys(page).sort().join(',')!=='ciphertext,commit,contract,iv,snapshot_id'||page.contract!==BACKUP_CONTRACT||page.commit!==commit||page.snapshot_id!==snapshot_id)fail();
  const iv=unb64(page.iv);if(iv.length!==12||typeof page.ciphertext!=='string'||page.ciphertext.length>9500000)fail();
  const raw=await crypto.subtle.decrypt({name:'AES-GCM',iv,additionalData:enc.encode(BACKUP_CONTRACT+'\n'+commit+'\n'+snapshot_id),tagLength:128},await backupKey(secret),unb64(page.ciphertext));
...
export async function archiveBackupPage(env,storage,request){
  ...
  if(!request||typeof request!=='object'||Array.isArray(request)||Object.keys(request).some(k=>!['part','table','offset','after'].includes(k)))fail();
  let page;
  if(request.part==='catalogue'){
    ...
    page={part:'catalogue',counts,schema,digests,schema_sha256,state_sha256,
    ...
    if(Object.keys(request).sort().join(',')!=='offset,part,table'||!ARCHIVE_TABLES.includes(request.table)||!Number.isSafeInteger(request.offset)||request.offset<0||request.offset>100000)fail();
    // All governed tables currently have rowid. No arbitrary identifier or SQL.
    const values=(await env.REVIEW_DB.prepare(`SELECT * FROM ${request.table} ORDER BY rowid LIMIT 100 OFFSET ?`).bind(request.offset).all()).results;
    page={part:'sql',table:request.table,offset:request.offset,rows:values};
