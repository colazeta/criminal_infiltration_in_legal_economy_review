...
test('automatic loading traverses all 294 index rows with six bounded requests',async()=>{
 const records=Array.from({length:294},(_,i)=>({...record,id:'CAND-AUTO-'+i}));let calls=0;
 const v=register(async url=>{
  ...
  return ok(indexPage(records.slice(start,start+50).map(r=>indexRow(r,projection)),{total:294,next:start+50<294?start+50:null}));
 },records);
 await tick();assert.equal(calls,6);assert.equal(records.filter(r=>v.filter.describe(r).includes('Analisi AI')).length,294);
 assert.equal(v.button.hidden,true);
});
