...
test('known Cloudflare KV tables are accessed through adapters and do not masquerade as application schema',async()=>{
 ...
 const r=await auditArchitecture(x.runtime);assert.equal(r.integrity_verified,true);assert.equal(Object.keys(r.counts).length,38);
...
test('schema census reveals only public names and hashes unknown private names',async()=>{
 ...
 const census=await response.json();assert.equal(census.expected_present.length,38);
