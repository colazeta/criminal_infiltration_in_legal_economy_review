...
function setup() {
  ...
  db.sqlite.exec(`INSERT INTO legacy_snapshots VALUES ('TEST-SNAPSHOT','TEST-COMMIT','TEST-HASH','test/private/snapshot',1,'2026-09-08T00:00:00Z','2026-09-08T00:00:00Z');
    INSERT INTO reviews VALUES ('TEST-REVIEW','Synthetic test review','CILE-4PT-v2','0.2.0','active','Europe/Rome','2026-09-08','2026-09-08T00:00:00Z','TEST-SNAPSHOT','2026-09-08T00:00:00Z');
...
test("migration creates no scientific data and DOI-less bibliography is representable", () => {
  ...
  assert.throws(() => db.sqlite.exec("INSERT INTO reviews VALUES ('R','Test','CILE-4PT-v2','0.2.0','active','Europe/Rome','2026-09-08','2026-09-08T00:00:00Z','MISSING-SNAPSHOT','2026-09-08T00:00:00Z')"), /verified_complete_snapshot_required/);
});
...
test("approval must be human, on the exact head, and bind the exact payload hash", async () => {
  const p = { proposal_id: "TEST-PROPOSAL-01", payload_sha256: await sha256(canonicalJson(proposal())), protocol_version: "CILE-4PT-v2" };
