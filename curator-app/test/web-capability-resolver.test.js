...
test("only implemented guarded providers can become automatically eligible", () => {
  const manifest = webCapabilityManifest({
  ...
  const eligible = manifest.filter((provider) => provider.automaticEligible).map((provider) => provider.id);
  assert.deepEqual(eligible, ["jina_reader", "tavily_basic"]);
  assert.equal(manifest.find((provider) => provider.id === "exa").mode, "registered_only");
  assert.equal(manifest.find((provider) => provider.id === "serper").automaticEligible, false);
});
...
test("Jina Reader can resolve a DOI page before any search credit is consumed", async (context) => {
  withFetchMock(context, async (url) => {
    ...
    return new Response(
      `Title: ${title}\nURL Source: https://doi.org/${doi}\n\nAbstract This article studies legitimate businesses confiscated from mafia groups in Italy and explains geographic and social determinants of investment choices across sectors and territories. Introduction The article then presents data and methods.`,
      { status: 200 },
    );
  });
...
test("Tavily Basic is used only after the reader layer is unavailable or inconclusive", async (context) => {
  let calls = 0;
