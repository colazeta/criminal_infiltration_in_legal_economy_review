...
test('Durable Object initialises only enrichment tables and is inactive until readback activation',async()=>{const{core,db}=setup();await core.ready;assert.equal((await core.environment()).PAPER_ENRICHMENT_ENABLED,'false');assert.equal(db.prepare("SELECT COUNT(*) n FROM sqlite_master WHERE type='table'").get().n,46);assert.equal((await core.verify()).verified,true)});
