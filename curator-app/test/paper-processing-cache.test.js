...
test('the entry document invalidates earlier register bundle cache keys', () => {
  ...
  assert.match(html,/paper-register\.js\?v=completion-001/);
  assert.doesNotMatch(html,/paper-register\.js\?v=(?:research|processing)-001/);
});
...
test('both research-sheet imports invalidate the previous unversioned child bundle', () => {
  ...
  assert.equal((source.match(/import\("\.\/paper-sheet-research\.js\?v=completion-001"\)/g)||[]).length,2);
